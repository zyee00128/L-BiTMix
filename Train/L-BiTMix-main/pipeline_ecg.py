...
def load_pretrained_model(net, path, args, device='cpu'):
...
def MixTcn_Lite(args): 
    ...
    for fold_idx, (train_ds, valid_ds, test_ds) in enumerate(fold_datasets):
        ...
        elif args.op_config == 'HM_BiTCN_parallel' or args.op_config == 'HM_BiTCN_search' or args.op_config == 'HM_BiTCN':
        ...
        if args.load_pretrain:
            ...
            if os.path.exists(path + file_name_pretrain):
                net = load_pretrained_model(net, path + file_name_pretrain, args, device=device)
        
        optimizer = optim.AdamW(net.parameters(), lr=0.001)
        net.to(device)
        early_stopping = EarlyStopping(15, verbose=True,dataset_name=fold_save_name,delta=0, args=args)
        
        my_lr_scheduler = get_linear_schedule_with_warmup(optimizer,int(iteration*0.01), iteration, last_epoch=-1)
        scaler = GradScaler(device=args.device, enabled=args.enable_amp) if 'cuda' in str(device) else GradScaler(enabled=False)
        best_map = -1.0
        ...
        for current in range(iteration):
            ...
            with torch.no_grad():
                # 【核心修复】：接收 4 个返回值，丢弃后两个
                images, labels, _, _ = Cutmix(images, labels, device)
            
            with autocast(device_type=args.device.split(":")[0], enabled=args.enable_amp):
                outputs = net(images)
                loss = F.binary_cross_entropy_with_logits(outputs, labels)

            if args.enable_amp and 'cuda' in str(device):
