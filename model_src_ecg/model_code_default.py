...
class MyResidualBlock(nn.Module):
    def __init__(self,input_complexity,output_complexity,stride,downsample,rank_list,dropout_coef,information='fisher',mix_weight=0.5):
        super(MyResidualBlock,self).__init__()
        self.mix_weight=mix_weight
        # print('mix_weight in ResBlock: ',mix_weight)
        self.downsample = downsample
        self.stride = stride if self.downsample else 1
        K = 9
        P = (K-1)//2
        ## hidden mix layer: pointwise conv to mix channels (1x1)
        self.hidden_mix = nn.Conv2d(in_channels=output_complexity,
                                    out_channels=output_complexity,
                                    kernel_size=1, bias=False)
        self.bn_hidden = nn.BatchNorm2d(output_complexity)
        ## residual block
    ...
    def forward(self, x):
        ...
        mix = self.bn_hidden(self.hidden_mix(x))
...
class MyResidualBlock_woHM(nn.Module):
    def __init__(self,input_complexity,output_complexity,stride,downsample,rank_list,dropout_coef,information='fisher',mix_weight=0.5):
        super(MyResidualBlock_woHM,self).__init__()
        self.downsample = downsample
        self.stride = stride if self.downsample else 1
        K = 9
        P = (K-1)//2
    ...
    def merge_layer(self):
        ...
        self.conv3.merge()
        return
class NN_default(nn.Module): ## backbone model definition wo BiTCN
    def __init__(self,nOUT,complexity,inputchannel,num_layers=35,rank_list=32,information='fisher',num_encoder_layers=3,dropout_coef=0.2,mix_weight=0.5):
        super(NN_default,self).__init__()
        self.mix_weight=mix_weight
        print('mix_weight: ',mix_weight)
        stride = 4
        ...
        print ('num_classifier_layers: ', self.num_classifier_layers)  
        ...
        # ResidualBlock layers wo CM
        self.encoder_layers = nn.ModuleList(
                [MyResidualBlock_woHM(inputchannel, complexity, stride, downsample=True, rank_list=self.rank_list[0:3],information=information,dropout_coef=dropout_coef,mix_weight=self.mix_weight)] )
        self.encoder_layers += nn.ModuleList(
                [MyResidualBlock_woHM(complexity, complexity, stride, downsample=True, rank_list=self.rank_list[3*(i+1):3*(i+2)],information=information,dropout_coef=dropout_coef,mix_weight=self.mix_weight) for i in range(self.num_encoder_layers-1)])
        # # ResidualBlock layers wo BiTCN
        # self.encoder_layers = nn.ModuleList(
        #         [MyResidualBlock(inputchannel, complexity, stride, downsample=True, rank_list=self.rank_list[0:3],information=information,dropout_coef=dropout_coef,mix_weight=self.mix_weight)] )
        # self.encoder_layers += nn.ModuleList(
        #         [MyResidualBlock(complexity, complexity, stride, downsample=True, rank_list=self.rank_list[3*(i+1):3*(i+2)],information=information,dropout_coef=dropout_coef,mix_weight=self.mix_weight) for i in range(self.num_encoder_layers-1)])
        
        # BiTCN layers
        self.hm_bitcn = HM_BiTCN_Encoder(complexity, dilations=[8,4,2,1])
    ...
    def feature_extraction(self, x, semi_flag = False):
        ...
        x_1 = self.hm_bitcn(x).permute(0, 2, 1)

        x = x.permute(0, 2, 1)
        x_2 = self.position_encoding(x)
        for layer in self.transformer_layers:
            x_2 = layer(x_2)

        x = (x_1 + x_2).permute(0, 2, 1) ##for abluation study wo CM
        # x = x_2.permute(0, 2, 1) ##for abluation study wo BiTCN
...
class NN_default_series(nn.Module): ## backbone model definition
    def __init__(self,nOUT,complexity,inputchannel,num_layers=35,rank_list=32,information='fisher',num_encoder_layers=3,dropout_coef=0.2,mix_weight=0.5):
        super(NN_default_series,self).__init__()
        self.mix_weight=mix_weight
        print('mix_weight: ',mix_weight)
        stride = 4
        self.num_layers = num_layers
...
class NN_default_parallel(nn.Module): ## backbone model definition
    def __init__(self,nOUT,complexity,inputchannel,num_layers=35,rank_list=32,information='fisher',num_encoder_layers=3,dropout_coef=0.2,mix_weight=0.5):
        ...
        # BiTCN layers
        self.hm_bitcn = HM_BiTCN_Encoder(complexity, dilations=[8,4,2,1])
        # TCN Layers for comparation with unidirectional TCNs
        self.hm_tcn_layers = TCN_Encoder(complexity, dilations=[8,4,2,1])
    ...
    def feature_extraction(self, x, semi_flag = False):
        ...
        x_1 = self.hm_bitcn(x).permute(0, 2, 1)
        # x_1 = self.hm_tcn_layers(x).permute(0, 2, 1) # for comparation with unidirectional TCNs
...
class NN_default_replace(nn.Module): ## backbone model definition wo Transformer
    def __init__(self,nOUT,complexity,inputchannel,num_layers=35,rank_list=32,information='fisher',num_encoder_layers=3,dropout_coef=0.2,mix_weight=0.5):
        super(NN_default_replace,self).__init__()
        self.mix_weight=mix_weight
        print('mix_weight: ',mix_weight)
        stride = 4
        self.num_layers = num_layers
